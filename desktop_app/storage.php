...
try {
    include $_SERVER['DOCUMENT_ROOT'].'/desktop_app/headers.php';
    ...
    */
    ...
    if (IsModuleInstalled('disk')) {
        ...
        if ($diskEnabled && \Bitrix\Disk\Configuration::REVISION_API >= 5) {
            ...
            $storageController->setActionName($_REQUEST['action'])->exec();
        } else {
            $diskEnabled = false;
        }
    }
    ...
    if (!$diskEnabled) {
        $APPLICATION->IncludeComponent('bitrix:webdav.disk', '', ['VISUAL' => false]);
        CMain::FinalActions();
        die();
    }
} catch (Throwable $e) {
